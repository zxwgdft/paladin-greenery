var baseUrl = "http://localhost:19000";
...
(function ($) {
    ...
    $(document).ajaxError(
        function (event, xhr, options, exc) {
            switch (xhr.status) {
                case 403:
                    $.errorMessage(exc || "您没有权限访问该页面或执行该操作");
                    break;
                case 404:
                    $.errorMessage("您访问的资源不存在");
                    break;
                case 401:
                    // ajax请求返回未登录状态处理
    ...
    $.extend({
        login: function (username, password, callback) {
            $.postJsonAjax(baseUrl + '/organization/authenticate/user', {
    ...
    var token = $.getUrlVariable("token");
    if (token) {
        $.setToken(token);
    }
