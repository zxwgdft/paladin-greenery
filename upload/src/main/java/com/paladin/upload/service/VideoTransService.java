...
public class VideoTransService {
    ...
    private void initialize() {
        ...
        Thread thread = new Thread() {
            public void run() {
                while (true) {
                    try {
                        UploadVideo uploadFile = queue.take();
    ...
    private LinkedBlockingQueue<UploadVideo> queue = new LinkedBlockingQueue<>();

    public void put2transcode(UploadVideo uploadFile) {
        queue.offer(uploadFile);
    }

    private void transcode(UploadVideo uploadFile) {

        String id = uploadFile.getId();
        int status = uploadFileMapper.getTranscodeStatus(id);
        if (status != UploadVideo.TRANSCODE_STATUS_NONE) {
...
//                String vc = mi.getVideo().getDecoder();
                ...
                if (Files.exists(out)) {
                    uploadFileMapper.updateTranscodeStatus(id, UploadVideo.TRANSCODE_STATUS_SUCCESS);
                    log.info("转换视频[" + outPath + "]成功");

                } else {
                    uploadFileMapper.updateTranscodeStatus(id, UploadVideo.TRANSCODE_STATUS_ERROR);
